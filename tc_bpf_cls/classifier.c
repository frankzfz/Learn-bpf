...
static inline int is_http(struct __sk_buff *skb, __u64 nh_off) {
  ...
  /* ingress direction  and  egress"GET / HTTP/1.1"  HTTP */
  if (plength >= 11) {
    unsigned long p[7];
    unsigned long get_p[7];
    int i = 0;
    for (i = 0; i < 4; i++) {
        get_p[i] = load_byte(skb, poffset + i);
        //trace_printk("get_p[%d]=%x\n",i,get_p[i]);
    }
    if ((get_p[0] == 'G') && (get_p[1] == 'E') && (get_p[2] == 'T'))  {
        trace_printk("This is GET HTTP\n");
        return 0;
    } else {
        for (i = 0; i < 4; i++) {
